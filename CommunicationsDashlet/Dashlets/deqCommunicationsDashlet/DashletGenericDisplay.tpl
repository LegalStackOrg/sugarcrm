...
<table cellpadding='0' cellspacing='0' width='100%' border='0' class='list view'>
	...
	{foreach name=rowIteration from=$data key=id item=rowData}
        ...
        {if $pageData.access.edit}
					<a title='Assign Me' onclick="javascipt:YAHOO.util.Connect.asyncRequest('POST', 'index.php?entryPoint=ajax_request&method=assign_lead', {ldelim}success: function(o){ldelim}return SUGAR.mySugar.retrieveDashlet('{$dashletId}'); {rdelim}, failure: function(o){ldelim}return SUGAR.mySugar.retrieveDashlet('{$dashletId}'); {rdelim}, argument: {ldelim} record:'{$rowData.ID}', design_processed:'{$rowData.ID}'{rdelim}	{rdelim}, 'record={$rowData.ID}');" href='#'><img border="0" src="{sugar_getimagepath file="accept_inline.gif"}"></a>
				{/if}
				*}
				{if $pageData.access.edit}
